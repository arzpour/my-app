 ...
 * ============================================
 *
 * This file handles cheque operations. In the new backend:
 * - Cheques use dealId instead of ChassisNo
 * - Structure changed significantly (IChequeNew type)
 * - Type field: "issued" | "received" instead of "صادره" | "وارده"
 *
 * NEW API ENDPOINTS TO USE:
 * - GET /cheques - Get all cheques
 * - GET /cheques/deal/:dealId - Get cheques by deal ID (replaces byChassisNo)
 * - GET /cheques/person/:personId - Get cheques by person (payer or payee)
 * - GET /cheques/status/:status - Get cheques by status
 * - GET /cheques/unpaid/deal/:dealId - Get unpaid cheques (replaces unpaid by chassisNo)
 ...
 * - CustomerNationalID -> payer.nationalId or payee.nationalId
 * - Bank -> bankName
 * - SayadiID -> (check actions array or add as separate field)
 * - CarChassisNo -> relatedDealId (but need to convert VIN to dealId first)
 */
...
/**
 * MIGRATION: Replace with GET /cheques
 * Returns: IChequeNew[] instead of IChequeRes[]
 */
