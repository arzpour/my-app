...
import { createSalary } from "@/apis/client/salaries";
...
const SalariesForm: React.FC<SalariesFormProps> = ({
  onSuccess,
  embedded = false,
}) => {
  ...
  const onSubmit: SubmitHandler<salarySlipSchemaType> = async (data) => {
    try {
      ...
      toast.error(error?.response?.data?.message || "خطا در ثبت پرداخت حقوق");
    }
  };
  ...
  if (embedded) {
    return (
      <div dir="rtl">
        <div className="mb-4">
          <h2 className="text-xl font-bold">پرداخت حقوق</h2>
        </div>
        {formContent}
      </div>
    );
